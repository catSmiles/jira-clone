  ...
  const handleIssueDrop = ({ draggableId, destination, source }) => {
    ...
    api.optimisticUpdate(`/issues/${issueId}`, {
      updatedFields: {
        ...
        position: calculateIssuePosition(project.issues, destination, source, issueId),
      },
...
const calculateIssuePosition = (...args) => {
  ...
  if (!prevIssue && !nextIssue) {
    ...
    position = prevIssue.position + (nextIssue.position - prevIssue.position) / 2;
  }
  return position;
};
...
const getSortedListIssues = (issues, status) =>
  issues.filter(issue => issue.status === status).sort((a, b) => a.position - b.position);
